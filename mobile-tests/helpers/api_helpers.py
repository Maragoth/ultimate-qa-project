...
def create_article_via_api(token, article):
    ...
    response = requests.post(
        f"{API_URL}/articles", json={"article": article}, headers=headers
    )
    response.raise_for_status()
    data = response.json()
...
def follow_user_via_api(token: str, username: str) -> None:
    """Follow a user via API call"""
    response = requests.post(
        f"{API_URL}/profiles/{username}/follow",
        headers={"Authorization": f"Token {token}"},
    )
    response.raise_for_status()
    return response.json()
...
def unfollow_user_via_api(token: str, username: str) -> None:
    """Unfollow a user via API call"""
...
def get_feed_articles(token: str) -> list:
    """Get articles from user's feed"""
    response = requests.get(
        f"{API_URL}/articles/feed", headers={"Authorization": f"Token {token}"}
    )
    response.raise_for_status()
